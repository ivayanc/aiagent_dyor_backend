...
def get_github_account_type(account_name: str) -> Optional[str]:
    """
    ...
    """
    try:
        response = requests.get(f"https://api.github.com/users/{account_name}")
...
def get_github_repos_info(account_name: str) -> Optional[List[Dict]]:
    """
    ...
    account_type = get_github_account_type(account_name)
    if not account_type:
        print(f"Account {account_name} not found")
        return None
        
    if account_type == "organization":
        repos_url = f"{base_url}/orgs/{account_name}/repos"
    else:
        repos_url = f"{base_url}/users/{account_name}/repos"
    ...
    try:
        while True:
            ...
            for repo in repos:
                ...
                commits_url = f"{base_url}/repos/{account_name}/{repo['name']}/commits"
...
def format_repo_info(repos_info: List[Dict]) -> str:
    ...
    for repo in repos_info:
        result.append(
            ...
            f"Type: {'Fork' if repo['is_fork'] else 'Source'}\n"
